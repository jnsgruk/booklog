...
pub(crate) async fn admin_page(
    ...
    let users = if auth_user.is_admin {
        state
            .user_repo
            .list_all()
            .await
    ...
    let template = AdminTemplate {
        ...
        current_user_id: i64::from(auth_user.id),
        ai_usage,
        passkeys,
        tokens,
        users,
...
pub(crate) async fn extract_user_from_session(
    state: &AppState,
    cookies: &Cookies,
) -> Option<crate::domain::users::User> {
    let cookie = cookies.get("booklog_session")?;
