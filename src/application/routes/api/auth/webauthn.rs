...
pub(crate) async fn auth_finish(
    ...
    let (user_id, cred_db_id, found_passkey) =
        find_credential_owner(&credentials, auth_result.cred_id())?;

    // Update credential counter if needed
    update_credential_counter(&state, cred_db_id, &auth_result, found_passkey).await;
...
fn find_credential_owner(
    credentials: &[crate::domain::passkey_credentials::PasskeyCredential],
    target_cred_id: &CredentialID,
) -> Result<
    (
        crate::domain::ids::UserId,
        crate::domain::ids::PasskeyCredentialId,
        Option<Passkey>,
    ),
    StatusCode,
> {
    for cred in credentials {
        ...
        if passkey.cred_id() == target_cred_id {
            return Ok((cred.user_id, cred.id, Some(passkey)));
        }
    }
...
async fn update_credential_counter(
    state: &AppState,
    cred_db_id: crate::domain::ids::PasskeyCredentialId,
    auth_result: &AuthenticationResult,
    found_passkey: Option<Passkey>,
) {
