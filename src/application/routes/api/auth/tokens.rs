...
pub async fn create_token(
    ...
    info!(token_id = %stored_token.id, token_name = %stored_token.name, user_id = %auth_user.effective.id, "API token created");
...
#[tracing::instrument(skip(state, auth_user), fields(token_id = %token_id, username = %auth_user.effective.username))]
pub async fn revoke_token(
    ...
    if token.user_id != auth_user.effective.id {
    ...
    info!(%token_id, user_id = %auth_user.effective.id, "API token revoked");
