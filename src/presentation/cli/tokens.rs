...
pub async fn run(client: &BooklogClient, cmd: TokenCommands) -> Result<()> {
    match cmd {
...
pub async fn create_token(client: &BooklogClient, cmd: CreateTokenCommand) -> Result<()> {
    ...
    println!("  export BOOKLOG_TOKEN={token}");
...
pub async fn list_tokens(client: &BooklogClient) -> Result<()> {
    let tokens = client.tokens().list().await?;
    print_json(&tokens)
}

pub async fn revoke_token(client: &BooklogClient, cmd: RevokeTokenCommand) -> Result<()> {
