...
pub use authors::{AuthorDetailView, AuthorOptionView, AuthorView};
pub use books::{
    AuthorBookCardView, BookDetailView, BookLibraryInfo, BookOptionView, BookReadingCardView,
    BookView, UserBookView,
};
pub use genres::{GenreDetailView, GenreOptionView, GenreView};
pub use readings::{QuickReviewView, ReadingDetailView, ReadingView};
pub use timeline::{
    TimelineEventDetailView, TimelineEventView, TimelineMonthView, TimelineReadingDataView,
};
...
impl StatsView {
    pub fn is_empty(&self) -> bool {
        self.library == 0 && self.wishlist == 0 && self.currently_reading == 0
    }
}
...
use crate::domain::books::books::BookAuthorInfo;
use crate::domain::listing::{DEFAULT_PAGE_SIZE, ListRequest, Page, PageSize, SortKey};

/// Joins author names with commas, falling back to "Unknown" when empty.
pub(super) fn format_author_label(authors: &[BookAuthorInfo]) -> String {
    if authors.is_empty() {
        "Unknown".to_string()
    } else {
        authors
...
/// Returns the display string for an optional value, or an em dash when `None`.
pub(super) fn or_em_dash(opt: Option<impl Display>) -> String {
    opt.map_or_else(
...
pub(super) fn author_path(id: impl Display) -> String {
    format!("/authors/{id}")
}

pub(super) fn book_path(id: impl Display) -> String {
    format!("/books/{id}")
}

pub(super) fn reading_path(id: impl Display) -> String {
    format!("/readings/{id}")
}

pub(super) fn genre_path(id: impl Display) -> String {
    format!("/genres/{id}")
}
...
impl<K: SortKey> ListNavigator<K> {
    ...
    /// Returns the base path without query or fragment.
