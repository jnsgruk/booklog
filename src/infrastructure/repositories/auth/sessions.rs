...
impl SqlSessionRepository {
    ...
    fn to_domain(record: SessionRecord) -> Session {
        ...
        Session::new(
            ...
            created_at,
            expires_at,
            acting_as_user_id.map(UserId::from),
        )
    }
}
...
impl SessionRepository for SqlSessionRepository {
    async fn insert(&self, session: NewSession) -> Result<Session, RepositoryError> {
        let query = "INSERT INTO sessions (user_id, session_token_hash, created_at, expires_at) VALUES (?, ?, ?, ?) RETURNING id, user_id, session_token_hash, created_at, expires_at, acting_as_user_id";
    ...
    async fn get(&self, id: SessionId) -> Result<Session, RepositoryError> {
        let query = "SELECT id, user_id, session_token_hash, created_at, expires_at, acting_as_user_id FROM sessions WHERE id = ?";
    ...
    async fn get_by_token_hash(&self, token_hash: &str) -> Result<Session, RepositoryError> {
        let query = "SELECT id, user_id, session_token_hash, created_at, expires_at, acting_as_user_id FROM sessions WHERE session_token_hash = ?";
    ...
    async fn set_acting_as(
        &self,
        id: SessionId,
        acting_as: Option<crate::domain::ids::UserId>,
    ) -> Result<(), RepositoryError> {
        query("UPDATE sessions SET acting_as_user_id = ? WHERE id = ?")
            .bind(acting_as.map(i64::from))
            .bind(i64::from(id))
            .execute(&self.pool)
            .await
            .map_err(|err| {
                RepositoryError::unexpected(format!("failed to set acting_as on session: {err}"))
            })?;
