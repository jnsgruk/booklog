...
/// Get path to the booklog binary
pub fn booklog_bin() -> String {
    // Cargo sets this for integration tests - works for both debug and release
    env!("CARGO_BIN_EXE_booklog").to_string()
}
...
pub fn create_token(name: &str) -> String {
    ...
    rt.block_on(async {
        ...
        // Ensure the test user is an admin (backup/reset require admin)
        #[allow(clippy::expect_used)]
        ...
        #[allow(clippy::expect_used)]
        let token_value =
            booklog::infrastructure::auth::generate_token().expect("Failed to generate token");
        let token_hash = booklog::infrastructure::auth::hash_token(&token_value);
...
/// Run a booklog CLI command and return the output
pub fn run_booklog(args: &[&str], env: &[(&str, &str)]) -> std::process::Output {
    let (address, _) = ensure_server_started().expect("Failed to start test server");

    let mut cmd = Command::new(booklog_bin());
    cmd.args(args);
    cmd.env("BOOKLOG_URL", &address);

    for (key, value) in env {
        cmd.env(key, value);
    }

    cmd.output().expect("Failed to run booklog command")
}
...
fn create_entity_cli(args: &[&str], token: &str, label: &str) -> String {
    let output = run_booklog(args, &[("BOOKLOG_TOKEN", token)]);
...
pub fn create_author(name: &str, token: &str) -> String {
    create_entity_cli(&["author", "add", "--name", name], token, "author")
}

pub fn create_genre(name: &str, token: &str) -> String {
    create_entity_cli(&["genre", "add", "--name", name], token, "genre")
}

pub fn create_book(title: &str, author_id: &str, token: &str) -> String {
    create_entity_cli(
        &["book", "add", "--title", title, "--author-ids", author_id],
        token,
        "book",
    )
}

pub fn create_reading(book_id: &str, status: &str, token: &str) -> String {
    create_entity_cli(
        &[
            "reading",
            "add",
