const DONUT_ICONS = {
  ...
  book: '<path d="M10.75 16.82A7.462 7.462 0 0 1 15 15.5c.71 0 1.396.098 2.046.282A.75.75 0 0 0 18 15.06V3.56a.75.75 0 0 0-.546-.721A9.006 9.006 0 0 0 15 2.5a9.006 9.006 0 0 0-4.25 1.065v13.255ZM9.25 4.565A9.006 9.006 0 0 0 5 2.5a9.006 9.006 0 0 0-2.454.339A.75.75 0 0 0 2 3.56v11.5a.75.75 0 0 0 .954.721A7.462 7.462 0 0 1 5 15.5a7.462 7.462 0 0 1 4.25 1.32V4.565Z" />',
  bookmark:
  ...
  render() {
    ...
    const colorFor = (i) => {
      const alpha =
        items.length === 1
          ...
          : (
              0.25 +
              0.75 * ((items.length - 1 - i) / (items.length - 1))
            ).toFixed(2);
    ...
    const segments = items.map((item, i) => {
      ...
      return `<circle cx="${cx}" cy="${cy}" r="${radius}" fill="none"
        stroke="${colorFor(i)}" stroke-width="${strokeWidth}"
    ...
    const legend = items.map((item, i) => {
      ...
      return `<div style="display:flex;align-items:center;gap:0.5rem">
        <span style="flex-shrink:0;width:0.5rem;height:0.5rem;border-radius:9999px;background:${colorFor(i)}"></span>
