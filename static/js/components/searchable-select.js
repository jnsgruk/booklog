customElements.define(
  ...
  class extends HTMLElement {
    ...
    _setup() {
      ...
      const creatable = this.hasAttribute("creatable");
      ...
      if (creatable) {
        ...
        createBtn.className =
          "w-full px-3 py-2 text-left text-sm font-medium text-accent hover:bg-surface-alt transition";
      ...
      if (hiddenName) this.appendChild(hiddenName);
      ...
      search.addEventListener("input", () => {
        ...
        if (creatable && createBtn) {
          const exactMatch = buttons.some(
            (btn) => btn.dataset.display.toLowerCase() === q,
          );
          if (q && !exactMatch) {
      ...
      const selectCreate = () => {
        ...
        this.dispatchEvent(
          new CustomEvent("change", {
            detail: { value: "", display: text, data: { created: "true" } },
            bubbles: true,
          }),
        );
      };
      ...
      search.addEventListener("keydown", (e) => {
        ...
        if (creatable && createBtn && createBtn.style.display !== "none") {
          visible.push(createBtn);
        }
